...
// Open the webpage
  ...
  console.log("Numbers of rows:",await headers.count())
  console.log("Numbers of columns:",await rows.count())
   ...
   await expect(rows).toHaveCount(4)
    ...
    // Expected: Keyboard | Electronics | $79 | 0 | Out of Stock
       ...
       await expect(thirdRowCells).toHaveText(['Keyboard','Electronics','$79','0',
        'Out of Stock' ]);
    ...
    // Expected: 4 rows of product data
     ...
     for(let r=0; r < rowCount; r++)
        {
       const cellValues= await rows.nth(r).locator('td').allInnerTexts() //Get all cell 
 ...
 console.log("Price of Mouse:",mousePrice)
 ...
 let tp = 0;
  ...
  for (let i = 0; i < tableData.length; i++) {
    ...
    tp = tp+Number(tableData[i][2].replace('$', ''));// Convert the price string 
        //to a number and add it to totalPrice
      }
    expect(tp).toBe(1456);
    ...
    // 12 & 13. Highest and Lowest price
     ...
     let maxPrice = Number(tableData[0][2].replace('$', '')); // Initialize maxPrice with 
  // the price of the first product

  let minPrice = Number(tableData[0][2].replace('$', '')); // Initialize minPrice with 
  // the price of the first product

  for (let i = 1; i < tableData.length; i++) {
    const price = Number(tableData[i][2].replace('$', '')); // Convert the price string 
    // to a number
