/* annotations
-----------------
Playwright includes five core built-in annotations that natively control test runner 
behavior

test.skip(): Marks the test as irrelevant and prevents Playwright from running it.
test.fail(): Informs Playwright that the test is expected to fail. If the test actually 
passes, the runner will throw an error.
test.fixme(): Bypasses execution because the test is known to be broken, crashing, or 
unstable.
test.slow(): Triples the default test timeout for resource-heavy operations.
test.only(): Focuses execution exclusively on this test, ignoring all other tests in the 
project
