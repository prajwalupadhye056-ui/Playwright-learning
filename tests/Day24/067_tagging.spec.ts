...
3. Run tests that belong to both sanity AND regression(AND)
...
4. Run tests that belong to either sanity OR regression(OR)
npx playwright test /tests/day24/076_tagging.spec.ts --project=chromium 
--grep "@sanity|@regression"

5. Run sanity tests that are NOT regression tests
npx playwright test /tests/day24/076_tagging.spec.ts --project=chromium --grep "@sanity" 
--grep-invert "@regression"
