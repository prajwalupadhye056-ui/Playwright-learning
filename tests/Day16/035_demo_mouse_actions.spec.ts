...
test.beforeEach('Mouse Actions', async ({ page }) => {
...
test('2.Right click/context click ', async ({ page }) => {
  ...
  // Right click action/ context click  --> opens the context menu
  await rightClickButton.click({ button: 'right' });  // this will perform the right click 
  // action. Other options: "left" | "right" | "middle" (optional)
  ...
  //Handle dialog is optional, but if you want to verify the dialog message, you can 
...
test('3.Mouse Hover interaction & tooltip', async ({ page }) => {
  ...
  expect(await hoverButton.getAttribute('title')).toBe("This is a tooltip"); // Tool tip 
  // verification
