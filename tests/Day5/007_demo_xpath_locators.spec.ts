...
test('1.Xpath demo with playwright',async({page}) =>
{
 await page.goto('https://demowebshop.tricentis.com/');
  ...
  let products=page.locator("//h2//a[contains(@href, 'computer')]")
    ...
    //await products.click() // Error: strict mode violation //trying to performn single 
    // action 
    // on group of elements
  ...
  // 6. XPath with last()
      ...
      //const wishList=page.locator("//div[@class='column my-account']//li[last()]")
      const wishListText=await page.locator("//div[@class='column my-account']//li[last()]")
      .textContent()
      expect(wishListText).toBe('Wishlist')


      // 7. XPath with position()
          const addressesText=await page.locator("//div[@class='column my-account']//li[position()=3]")
          .textContent()
        ...
        expect(twitterText).toBe('Twitter'); // Expect the second social media link 
        // to be "Twitter"
})
