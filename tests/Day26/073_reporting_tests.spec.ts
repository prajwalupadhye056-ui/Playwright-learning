...
test.describe('demowebshop User Login Tests', () => {
 ...
 test('@P1 @Sanity Login test with valid credentials', async ({ page }) => {
     ...
     test.skip('@P2 @Regression demowebshop search test', async ({ page }) => {
        await page.goto('https://demowebshop.tricentis.com/login');
        await page.locator('#small-searchterms').fill("laptop");  // fill the text 
            // in search box
        await page.locator("input[value='Search']").click();      // click on the 
            // button
        await expect.soft(page.locator('h2 a').nth(0)).toContainText("laptop", { ignoreCase:
                 true });
        });
