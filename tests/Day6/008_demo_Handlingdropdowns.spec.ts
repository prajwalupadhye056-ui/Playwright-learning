   ...
   await expect(countrySelect).toHaveValue('usa')
    ...
    // Select multiple options by values
       ...
       test('Sorted dropdown should already be in alphabetical order', async ({ page }) => {
          ...
          const optionsText = await dropdownOptions.allTextContents();  // Returns array 
          // of all the text values of the options in the dropdown
         console.log("Options in the dropdown:", optionsText);
          ...
          // Store the original list of option texts
             ...
             // original array and hence both originalList and sortedList will be same (sorted)
              
             const sortedList = [...optionsText].sort(); //(spread operator) → creates a new 
             //copy of the array
