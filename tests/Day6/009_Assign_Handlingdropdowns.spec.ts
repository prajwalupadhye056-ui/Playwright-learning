...
test('Product sort and print lowest/highest price with names',async({page})=>
{
   ...
   const orderdropdown= page.locator('div.sort>select')

   await expect(orderdropdown).toBeVisible() // Assert dropdown is visible
   await expect(orderdropdown).toBeEnabled() // Assert dropdown is enabled
   await orderdropdown.selectOption('Lowest to highest')
...
//Finds all matching elements.(allInnerTexts())
// Gets the visible text from each element.
// Returns the values as an array of strings.
  ...
  console.log(`Lowest priced Products: ${names[0]}: ${prices[0]}`)
  console.log(`Highest priced products:${names[names.length-1]}: ${prices[prices.length-1]}`)
