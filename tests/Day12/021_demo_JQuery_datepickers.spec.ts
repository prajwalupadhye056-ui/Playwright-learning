...
async function selectDate(page: Page, targetYear: string, targetMonth: string, targetDay: 
  string, isFuture?: boolean) {
...
await dateInput.fill('08/15/2026') //    mm/dd/yyyy format
...
//Approach2: select date through calender
 ...
 //clicking on date picker
  ...
  await selectDate(page, targetYear, targetMonth, targetDay, true) // false = past date, 
  // true = future date

  //await selectDate(page, targetYear, targetMonth, targetDay) // for selecting current 
  // date, no need to pass flag true/false
