...
test('Date Range Picker ', async ({ page }) => {
  ...
  await page.goto('https://sdetqa.vercel.app/autoplay.html');
   ...
   //The error Malformed value when using locator.fill("20-10-2025") occurs because the 
  // <input type="date"> expects a date in the format YYYY-MM-DD — not DD-MM-YYYY.

  /*await page.locator('#start-date').fill('20-10-2025'); //DD-MM-YYYY
  await page.locator('#end-date').fill('05-09-2026'); //DD-MM-YYYY
   */

  await startDate.fill("2027-07-05") //YYYY-MM-DD
  await endDate.fill("2026-07-10")  //YYYY-MM-DD
   ...
   console.log("string:",await endDate.inputValue())


   await expect(startDate).toHaveValue('2027-07-05') //YYYY-MM-DD
