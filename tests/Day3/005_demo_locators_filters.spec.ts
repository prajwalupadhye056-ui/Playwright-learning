...
test.only('2.Count items not having "Out of stock" (in Stock)', async ({ page }) => {

    /*  const inStockitems= page.getByRole('listitem')
                        .filter({hasText:'In stock'}) */

    const inStockitems = 
        page.locator(".card").nth(1)   // css locators  go to second card then find list 
        // items in it
        .getByRole('listitem')
...
test('5.Count all elements with test ids', async ({ page }) => {
    ...
    console.log("Fruits......", await firstElement.innerText(), await lastElement.innerText()
...
test('7.Find "Say hello" button for Mary', async ({ page }) => {
   ...
   const HelloButton=page.getByRole('listitem')
                    .filter({hasText:'Mary'})
...
test('8.Find "Subscribe" buttons using multiple conditions', async ({ page }) => {
    ...
    const subscribeButtons = page.getByRole('button')   // condition1
...
test('9.Find "details" buttons for done tasks', async ({ page }) => {
    ...
    const doneTaskDetails = page.getByRole('listitem')
        .filter({ hasText: 'done' })
