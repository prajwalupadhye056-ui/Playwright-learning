...
- Name: Day25\072_flaky_test.spec.ts >> flaky test
- Location: tests\Day25\072_flaky_test.spec.ts:6:5
...
> 14 |     await expect(page.getByRole('link', { name: 'Log out' })).toBeVisible();
     |                                                               ^ Error: expect(locator).toBeVisible() failed
  15 |     await expect(page.locator('#nameofuser')).toContainText('Welcome pavanol');
  16 | 
  17 | });
  18 | 
  19 | 
  20 | // //Specify retries programatically
  21 | // // ---------
  22 | // test.describe.configure({retries: 3});
  23 | 
  24 | // test.describe('Unstable Feature Group', () => {
  25 | //     test.only('flaky test', async ({ page }) => {
  26 | 
  27 | //         await page.goto('https://www.demoblaze.com/index.html');
  28 | //         await page.getByRole('link', { name: 'Log in' }).click();
  29 | //         await page.locator('#loginusername').fill('pavanol');
  30 | //         await page.locator('#loginpassword').fill('test@123');
  31 | //         await page.getByRole('button', { name: 'Log in' }).click();
  32 | //         await page.waitForTimeout(10000);
  33 | //         await expect(page.getByRole('link', { name: 'Log out' })).toBeVisible();
  34 | //         await expect(page.locator('#nameofuser')).toContainText('Welcome pavanol');
  35 | 
  36 | //     });
  37 | // });
  38 | 
